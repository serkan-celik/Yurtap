...
@Injectable()
export class OgrenciGuard extends BaseComponent implements CanActivate {//CanActivate: gitmek istediği sayfa active edebilmiş mi? true:false
    ...
    canActivate(
        ...
        if (this.getUserRoles) {
            for (let index = 0; index < this.getUserRoles.length; index++) {
                if (isLoggedIn && ((this.getUserRoles[index].rolId == RolEnum.OgrenciYonetimi)
