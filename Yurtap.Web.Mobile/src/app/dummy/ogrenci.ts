...
export class DummyOgrenciListesi{

  private dummyOgrenciListesi:OgrenciListe[]  = 
...
getDummyOgrenciListesi() : Observable<OgrenciListe[]> {
    return Observable.create( observer => {
        observer.next(this.dummyOgrenciListesi);
        observer.complete();
    });}

    addDummyOgrenciListesi(ogrenci:Ogrenci) : Observable<Ogrenci> {
        return Observable.create( observer => {
            observer.next(this.dummyOgrenciListesi.push(ogrenci));
            observer.complete();
        });}
}
